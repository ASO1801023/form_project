...
def list_2(request):
    ...
    if 'newButton' in request.POST:
        IdeaTree(name="新しいプロジェクト", overview="", complete_flag="0", idea_theme=newTheme, lastidea_id="0", user_id=cre, passcode=code).save()
...
class ProfileView(LoginRequiredMixin, generic.View):
    ...
    def post(self,request,*args, **kwargs):
        ...
        if form.is_valid():
            ...
            user_obj = User.objects.filter(username=request.user.username).order_by("-id").first()
